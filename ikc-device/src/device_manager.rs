...
pub fn select_isd() -> Result<String> {
    let res = block_on(send_apdu("00A4040000".to_string()))?;
...
pub fn get_se_id() -> Result<String> {
    select_isd()?;
    let res = block_on(send_apdu("80CB800005DFFF028101".to_string()))?;
    ApduCheck::check_response(res.as_str())?;
...
pub fn get_sn() -> Result<String> {
    select_isd()?;
    let res = block_on(send_apdu("80CA004400".to_string()))?;
    ApduCheck::check_response(res.as_str())?;
